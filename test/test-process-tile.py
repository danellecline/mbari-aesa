...
if __name__ == '__main__':
    ...
    opts = ' --logverb=Debug --mbari-save-event-num=all --mbari-max-event-area=50000 --mbari-max-event-area=500 --mbari-cache-size=0 ' \
           ...
           '--vc-chans=O --mbari-save-boring-events=False --shape-estim-mode=ConspicuityMap ' \
           '--use-older-version=false --ior-type=ShapeEst --maxnorm-type=FancyOne --mbari-saliency-input-image=Raw ' \
           '--mbari-max-evolve-msec=20000 --boring-sm-mv=0.5 --mbari-use-foa-mask-region=False ' \
           '--mbari-max-event-frames=1 --mbari-save-original-frame-spec=true --mbari-save-output ' \
           '--levelspec=0-3,2-5,1 --mbari-max-WTA-points=20 --num-orient=32 --ge-type=FFT '
    #-levelspec=1-3,2-4,1'
    #--levelspec=1-3,2-4,1'
    #--levelspec=0-3,2-5,2
    cnt = 0
    ...
    for f in filenames:
        ...
        # split along whatever is the largest dimension: height or width
        if width > 10*height:
            horiz_tiles = 20
            vert_tiles = 1
        else:
        ...
        for i in range(0, horiz_tiles*vert_tiles - 1):
            ...
            % (bin, opts, display, saliency_output, width/2, height/2, in_file, out_stem, out_stem, out_stem,
            ...
            os.system(cmd)
            os.system('convert %s-results%06d.pnm %sf-results%06d.jpg' % (out_stem, 0, out_stem, i))
            os.remove('%s-results%06d.pnm' % (out_stem, 0))
            os.system('convert %s-T%06d.pnm %sf-saliency-map%06d.jpg' % ( out_stem, 0, out_stem, i))
            ...
            for name in sorted(glob.glob(os.path.join(os.getcwd(), out_stem + '*evt*.pnm'))):
                ...
                stem = tail.split('.')[0]
                out_file = '%s.jpg' % stem
                os.system('convert name %s' % out_file)
                os.remove(name)
